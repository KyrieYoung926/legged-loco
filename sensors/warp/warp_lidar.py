...
import warp as wp
import copy
from legged_gym.sensors.warp.warp_kernels.warp_lidar_kernels import LidarWarpKernels
...
class WarpLidar:
    def __init__(self, num_envs, config, mesh_ids_array, device="cuda:0"):
        ...
        self.lidar_position_array = None#wp.from_torch(torch.zeros(self.num_envs, self.num_sensors, 3, device=self.device),dtype=wp.vec3)
        self.lidar_quat_array = None#wp.from_torch(torch.zeros(self.num_envs, self.num_sensors, 4, device=self.device),dtype=wp.quat)
        self.graph = None
        # self.test_count = np.array([0], dtype=np.uint64)
    ...
    def capture(self, debug=False):
        # if torch.isnan(self.pixels).any() or torch.isinf(self.pixels).any():
        #     raise ValueError("self.pixels contains NaNs or Infs")
        print(f"---lidar_position_array:",self.lidar_position_array)

        # print("lidar_quat_array:",self.lidar_quat_array)
