import warp as wp
import time 
from legged_gym.sensors.base_sensor import BaseSensor
...
from legged_gym.sensors.warp.warp_lidar import WarpLidar

from legged_gym.utils.logging import CustomLogger
...
class WarpSensor(BaseSensor):
    ...
    def init_tensors(self, global_tensor_dict):
        ...
        self.reset()
        #self.expand_test = self.pixels.norm(dim=4, keepdim=True).expand(-1, -1, -1, -1, 3)
    ...
    def update(self):
        ...
        self.robot_position = self.global_tensor_dict["robot_position"].unsqueeze(1).expand(-1, self.num_sensors, -1)
        self.robot_orientation = self.global_tensor_dict["robot_orientation"].unsqueeze(1).expand(-1, self.num_sensors, -1)
        ...
        #print("Sensor position: ", self.sensor_position)
        ...
        # time.sleep(0.2)
        # if torch.isnan(self.pixels).any() or torch.isinf(self.pixels).any():
        #     raise ValueError("self.pixels contains NaNs or Infs")

        self.sensor.capture()

        # Simplify the operation and check if the tensor can be moved to CPU
    ...
    def apply_range_limits(self):
        ...
        # try:
        #     c = self.pixels.cpu()
        #     print("Tensor successfully moved to CPU")
        # except RuntimeError as e:
        #     print("Error moving tensor to CPU:", e)
