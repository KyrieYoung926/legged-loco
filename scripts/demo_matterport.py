...
parser.add_argument(
    "--scene", default="go2_matterport", choices=["go2_matterport", "h1_matterport"], type=str, help="Scene to load."
)
...
class Planner:
    def __init__(self, env_cfg, args_cli, simulation_app):
        ...
        self.env = gym.make(args_cli.task, cfg=env_cfg, render_mode=None)

        if args_cli.history_length > 0:
            self.env = RslRlVecEnvHistoryWrapper(self.env, history_length=args_cli.history_length)
        else:
            self.env = RslRlVecEnvWrapper(self.env)

        self.vel_command = torch.tensor([0.0, 0.0, 0.0], device = self.env.device)
        ...
        self.identity_quat = torch.tensor([1.0, 0.0, 0.0, 0.0], device=self.env.device).repeat(1, 1)
        self.data_npy = []
        ...
        for i in range(self.env_cfg.expert_path_length):
            ...
            point = np.array(env_cfg.expert_path[i]).reshape(1, 3)
            default_scale = expert_point_visualizer.cfg.markers["cuboid"].scale
            # import ipdb; ipdb.set_trace()
            larger_scale = 2.0*torch.tensor(default_scale, device=self.env.device).repeat(1, 1)
    ...
    def start_loop(self):
        ...
        for _ in range(20):
            obs, _, _, infos = self.env.step(torch.zeros((1, 12), device=self.env.unwrapped.device))
        ...
        cam_eye = (robot_pos_w[0]+0.0, robot_pos_w[1]+2.0, robot_pos_w[2]+2.5)
        ...
        agent_cfg: RslRlOnPolicyRunnerCfg = cli_args.parse_rsl_rl_cfg(self.args_cli.task, self.args_cli)

        log_root_path = os.path.join("logs", "rsl_rl", agent_cfg.experiment_name)
        log_root_path = os.path.abspath(log_root_path)
        # import pdb; pdb.set_trace()
        resume_path = get_checkpoint_path(log_root_path, self.args_cli.load_run, agent_cfg.load_checkpoint)

        ppo_runner = OnPolicyRunner(self.env, agent_cfg.to_dict(), log_dir=None, device=agent_cfg.device)  # Adjust device as needed
        ppo_runner.load(resume_path)

        self.policy = ppo_runner.get_inference_policy(device=self.env.unwrapped.device)
        ...
        while self.simulation_app.is_running(): # 20hz
            ...
            self.data_npy.append({'image': rgb_image_np, 'proprio_go2': proprio_go2, 'action': action.clone().detach().cpu().numpy(), 
                                  'language_instruction': env_cfg.instruction_text})
            ...
            if reached_goal_time > 10 or sim_t > max(env_cfg.expert_time + 10):
                break
...
if __name__ == "__main__":
    ...
    dataset_file_name = os.path.join(ASSETS_DIR, "vln-ce/R2R_VLNCE_v1-3_preprocessed/train/train_filtered.json.gz")
    # scene_id = None
    ...
    # if i != 0:
    #     # launch omniverse app
    #     app_launcher = AppLauncher(headless=args_cli.headless, offscreen_render=args_cli.offscreen_render)
    #     # import ipdb; ipdb.set_trace()
    #     simulation_app = app_launcher.app
    planner = Planner(env_cfg, args_cli, simulation_app)
    planner.start_loop()
    # Close the simulator
